...
class FilePickerActivity : AppCompatActivity() {
    ...
    private fun filePicker() {
        val mimeTypes : Array<String> = arrayOf("image/*", "video/*", "application/pdf", "audio/*", "text/*", "application/*")
    ...
    private fun send() {
        if (fileURI == "") {
            toast("Camara elije un archivo")
        } else {
            // Double check with user
            val alertDialogBuilder = AlertDialog.Builder(this)
            alertDialogBuilder.setTitle("Confirmación de envio")
            alertDialogBuilder.setMessage("Seguro quieres enviar este archivo?")
            alertDialogBuilder.setPositiveButton(R.string.send) { _, _ -> checkLessThan5MB(fileURI) }
            alertDialogBuilder.setNegativeButton(R.string.cancel) { _, _ -> toast("Envio cancelado") }
    ...
    private fun checkLessThan5MB(fileURI : String) {
        ...
        if (file.readBytes().size > (1024 * 1024 * 5)) {
            ...
            val alertDialogBuilder = AlertDialog.Builder(this)
            alertDialogBuilder.setTitle("Archivo muy grande")
            alertDialogBuilder.setMessage("Este archivo pasa los 5MB de limite")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 111) {
            if (resultCode == RESULT_OK) {
                ...
                toast("Error al elegir el archivo")
            }
        }
    }
}
